...
// the "B" side.
//
// Each message can contain up to 32 bytes. The `length` field indicates how
// many of the bytes in `data` are actually valid. Note: every packet
// transmitted between the two entities will 32 bytes of data, but not all of it
// may be valid.
struct msg {
  int length;
  char data[32];
...
// A `pkt` is the data unit passed from layer 4 (the "A" and "B" entities) to
// layer 3 (the simulator). The `pkt` structure is pre-defined and may not be
// changed.
struct pkt {
  int seqnum;
  int acknum;
